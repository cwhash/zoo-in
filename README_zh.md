...
- 管理員後台網址是 `/admin`，Life Grid 2027 設定頁是 `/admin/life-grid-2027`。
...
活動代碼不放在前端程式碼。管理員在 `/admin/life-grid-2027` 設定代碼後，後端會正規化並轉成 SHA-256 hash，寫入 `activity_code_hashes/{code_hash}`，並用 transaction 限制 Life Grid 最多 999 人。
